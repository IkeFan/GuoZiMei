...
interface CommentHelper {
    fun Context.checkData(views: Array<TextView>, hints: Array<String>, submitData: () -> Unit ,vararg preCondition:()->Boolean) {
        for (i in 0 until views.size) {
            val str = views[i].text.toString().trim()
            if (str.isEmpty()) {
                ToastUtils.showShort(hints[i])
                return
            }
        }
        preCondition.forEach { it() }
        submitData()
    }
    ...
    fun String.showToast(frameApp: FrameApp?) {
        frameApp?.handler?.post { Toast.makeText(frameApp, this, Toast.LENGTH_SHORT).show() }
    }
